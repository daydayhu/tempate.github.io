...
Wallet.generate = function(password, walletName, pwdPrompt, callback, device) {
  ...
  /*
  *  需要包
  *  bip39、crypto、hdkey、ethereumjs-utilkeythereum
  *
  *  步骤：
  *  1.生成助记词。 依赖bip39、crypto
  *  2.生成公钥、私钥、钱包地址。 依赖ethereumjs-wallet/hdkey、ethereumjs-util
  *  3.创建keyStore。 依赖keythereum
  *
  * */
    ...
    if (walletsObj !== [] && walletsObj.length > 0) {
      for (var i = 0; i < walletsObj.length; i++) {
        if (walletsObj[i].walletName === walletName) {
          callback('The purse of repeat', null);
          return;
        }
      }
    }

    var mnemonic = bip39.entropyToMnemonic(crypto.randomBytes(16).toString('hex'));
    if (!bip39.validateMnemonic(mnemonic)) { callback('create failed', null); }
...
Wallet.importMnemonic = function(password, mnemonic, callback, device) {
  ...
  *   import ethUtil from 'ethereumjs-util';
      ...
      1.校验助记词。 依赖 ethereumjs-util、bip39、bitcoinjs-lib
      2.生成公钥、私钥、地址。依赖ethereumjs-wallet/hdkey
      3.生成keyStore。 依赖keythereum

  * */
  try {
    if (!Validator.isValidMnemonic(mnemonic)) {
      if (callback) {callback(INVALID_MNEMONIC, null);}
      return false;
    }
    var key = exportKeyByMnemonic(mnemonic, password);
    var dk = getSaltIv();
    ...
    if (wallet === null) {
      wallet = new Wallet(publicKey, 'ETH-' + new Date().toLocaleString(), '');
    } else {
      wallet = new Wallet(publicKey, wallet.walletName, wallet.pwdPrompt);
    }
    addNewWallet(wallet, keystore);
    if (callback) {callback(null, wallet);}
    // 向平台注册新用户
    sendRegiste(publicKey, device);
  } catch (err) {
    console.error('导入助记词失败' + err);
    callback(err, null);
  }
};
/**
...
Wallet.importPrivateKey = function(password, privateKey, callback, device) {
  try {
    if (!Validator.isValidPrivKey(privateKey.length)) {
      if (callback) {callback(INVALID_PRIVATE_KEY, null);}
      return false;
    }
    var dk = getSaltIv();
...
Wallet.importFromKeyStore = function(password, input, callback, device) {
  try {
    ...
    console.error('导入KeyStore失败' + err);
    callback(err, null);
  }
};
/**
...
Wallet.prototype.getTransactionHistory = function(id, contactAddress, symbol, decimals, callback) {
  ...
  ServerApi.getHistory(this.address, contactAddress, id, PAGE_SIZE, function(err, response) {
    ...
    if (callback) callback(null, historys);
  });
};
/**
 * 查询当前钱包的消息
