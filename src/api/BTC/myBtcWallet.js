...
import {EthUnits} from '../ETH/ethUnits';
...
BtcWallet.importFromKeyStore = function(password, input, callback, device) {
  ...
  try {
    ...
    if (wallet === null) {
      wallet = new BtcWallet(address, 'BTC_' + name, '');
...
function encryptPrivate(wifString, password) {
  ...
  // var decoded = wif.decode(wifString, net.wif);
  //
  ...
  var Crypto2 = ezcrypto.Crypto;
  var crypted = Crypto2.MARC4.encrypt(wifString, password);
  console.log('crypted===', crypted);
  console.log('password===', password);
  console.log('text===', wifString);
  return crypted;
}
...
function decryptPrivate(encryptedKey, password) {
  ...
  // });
  // // var wifPrivate = wif.encode(mainVersion, decryptedKey.privateKey, decryptedKey.compressed);
  // var wifPrivate = wif.encode(testVersion, decryptedKey.privateKey, decryptedKey.compressed);
  // console.log(wifPrivate);
  // return wifPrivate;

  var Crypto2 = ezcrypto.Crypto;
  var crypted = Crypto2.MARC4.decrypt(encryptedKey, password);
  return crypted;
}
